...
@Service
@Profile("plus")
public class PlusServiceImpl implements ShopService {

    private static final Logger logger = LoggerFactory.getLogger(PlusServiceImpl.class);
    ...
    public BigDecimal sumUpPrices() {
        ...
        BigDecimal result = total.add(taxTotal);
        logger.info("After taxes: " + currency.format(result) + " = " + currency.format(total) + " (prices) + " + currency.format(taxTotal) + " (taxes)");
